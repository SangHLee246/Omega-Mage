...
public class LayoutTiles : MonoBehaviour {
	...
	void Start () {
		...
		//all Ties). This keeps Tiles tidy in the Hierarcy pane.
	...
	//Build a based on room number. This is an alternative version of
	//Build room that grabs roomXML based on <room> num.
	...
	public void BuildRoom(PT_XMLHashtable room) {
		...
		for (int y = 0; y<roomRows.Length; y++) {
			...
			//If the type is still rawType, continue to the next iteration
				...
				case "X": //Starting position for the Mage
					//Mage.S.pos = ti.pos; //Uses the Mage Singleton COMMENT OUT THIS LINE!
				if (firstRoom) {
				...
				case "F":
					...
					//^Attaching this to the tileAnchor meanas that the Portal
		...
		foreach (Portal p in portals) {
			...
			if (p.toRoom == roomNumber || firstRoom) {
				...
				//_Mage mantains her facing from the previous room, so there
